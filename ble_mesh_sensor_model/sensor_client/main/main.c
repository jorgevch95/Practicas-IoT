...
uint16_t server_address[] = {
    [0] = ESP_BLE_MESH_ADDR_UNASSIGNED,
    [1] = ESP_BLE_MESH_ADDR_UNASSIGNED,
    [2] = ESP_BLE_MESH_ADDR_UNASSIGNED,
    [3] = ESP_BLE_MESH_ADDR_UNASSIGNED,
    [4] = ESP_BLE_MESH_ADDR_UNASSIGNED,
    [5] = ESP_BLE_MESH_ADDR_UNASSIGNED
};
                               ...
                               uint16_t primary_addr, uint8_t element_num, uint16_t net_idx)
{
    ...
    cont++;
    server_address[cont] = primary_addr;
...
void example_ble_mesh_send_sensor_message(uint32_t opcode, uint32_t indx)
{
    ...
    node = esp_ble_mesh_provisioner_get_node_with_addr(server_address[indx]);
    if (node == NULL) {
        ESP_LOGE(TAG, "Node 0x%04x not exists", server_address[indx]);
        return;
    }
...
static void example_ble_mesh_sensor_timeout(uint32_t opcode)
{
    ...
    example_ble_mesh_send_sensor_message(opcode, cont);
}
